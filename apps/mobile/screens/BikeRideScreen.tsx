...
export default function BikeRideScreen({ navigation, route }: RootStackScreenProps<'BikeRide'>) {
    ...
    const { energy: energyStore, setEnergy: setEnergyStore, tokens, setTokens} = useEnergyTokensStore();
        ...
        const inference = async () => {
            ...
            if (res[0] > LOWER_LOSS_LIMIT && res[0] < UPPER_LOSS_LIMIT) {
                setTravelledDistance(
                    (prev) => prev + travelledDistanceBuffer.current
                );
                setIsBicycle(true);
            } else {
                setIsBicycle(false);
    ...
    useEffect(
        () =>
            navigation.addListener('beforeRemove', (e) => {
                ...
                Alert.alert(
                    'End the ride?',
                    'Your ride is still on. Do you want to end it?',
                    [
                        {
                            text: "Don't leave",
                            style: 'cancel',
    ...
    useEffect(() => {
        if (isEnded) {
            navigation.replace('SummaryScreen', { distance: travelledDistance, time: seconds, earnedTokens: earnedTokens  });
        }
    }, [isEnded])
                    ...
                    >
                        ...
                        <View style={{ alignItems: 'center' }}>
                            ...
                            <Text style={styles.stat}>
                                {`${Math.floor(seconds / 60) % 60}`.padStart(
                                    2,
                                    '0'
                                ) +
                                    ':' +
                        ...
                        <View style={{ alignItems: 'center' }}>
                            ...
                            <Text style={styles.stat}>
                                {`${Math.floor(
                                    ...
                                    '.' +
                                    `${
                                        Math.floor(travelledDistance / 10) % 100
                                    }`.padStart(2, '0')}
                            </Text>
                        </View>
                        <View style={{ alignItems: 'center' }}>
